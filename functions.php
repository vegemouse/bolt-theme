...
function bolt_setup() {
	...
	register_nav_menus( array(
		'menu-1' => esc_html__( 'Primary', 'bolt' ),
	) );
...
function bolt_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'bolt_content_width', 640 );
}
...
function bolt_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'bolt' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'bolt' ),
...
function bolt_scripts() {
	...
	wp_enqueue_script( 'bolt-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'bolt-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
function isa_order_received_text( $text, $order ) {
    $new = $text . ' <p style=\'color: #aa0000\'>Please Note: this is not an actual purchase. Your order list will be sent to the Vancouver Bolt sales department. A rep will reach out to you with estimated pricing and process the order when you are ready. If you have any questions or concerns, please email us at: onlinesales@vancouverbolt.com  or call us at: (360) 699-4406.</p>';
    return $new;
}
